...
class CreateCourse extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      courseRelationAlreadyExistsMessage:
        'The selected already exists, Please select a differnet course',
      alertType: 'error',
  ...
  getDataToRenderPage = () => {
    ...
    fetchApi('/courses', {})
    .then(response => {
      allCourses = response.availableCourses;
      authenticatedFetchAPI('/courseRelation', {})
        .then(response => {
  ...
  makeDataDisplayReady = (allCourses, allCourseRelations) => {
    ...
    this.setState({
      ...
      allCourseRelations: allCourseRelations,
    });
  };
  ...
  mergeArrays = (allCourses, allCourseRelations) => {
    ...
    allCourses.map(course => {
      allCourseRelations.map(courseRelation => {
        course.id === courseRelation.courseId
          ? course.reliesOn.push({
              courseId: courseRelation['reliesOn'],
              courseName: courseRelation['relatedCourseName'],
            })
          : '';
      });
      return course;
  ...
  deleteCourseRelation = (e, course, relatedCourse) => {
    ...
    deleteCourseRelaionAPI(parentCourseId, reliesOn)
      .then(response => {
    ...
    this.setState(function(state, props) {
      let updateCourses = state.allCourses.map(courseState => {
        courseState.id === courseId
          ? (course['displayCourseDepenedencySelect'] = true)
          : '';
  ...
  addDependencyCourse = (course, event) => {
    ...
    let parentCourseId = course['id'];
    let isSelectedCourseValid = this.checkIsSelectedCourseValid(
      course,
      selectedCourse,
    );
    if (isSelectedCourseValid) {
      addCourseRelaionAPI(parentCourseId, selectedCourse)
        .then(response => {
  ...
  displayAddCourseRelationErrorMessage(response) {
    let message = response.message
      ? response.message
      : 'Course relation could not be added due to an error';
  ...
  checkIsSelectedCourseValid = (course, selectedCourseId) => {
    ...
    if (parseInt(parentCourseId) === parseInt(selectedCourseId)) {
      ...
      dependencyCourseIds &&
      _.contains(dependencyCourseIds, selectedCourseId)
    ) {
  ...
  render() {
    ...
    const {
      ...
      allCourseRelations,
      isDataFetched,
      showSameParentNotification,
      sameAsParentMessage,
      showCourseRelationAlreadyExists,
      courseRelationAlreadyExistsMessage,
      alertType,
      showAddCourseRelationError,
      AddCourseRelationErrorMessage,
      pageLoadError,
    ...
    if (!isDataFetched && pageLoadError) {
      return <div>{pageLoadErrorMessage}</div>;
                    ...
                    <TableCell scope="row">
                      ...
                      {course.reliesOn.map(relatedCourse => (
                        <Link
                          ...
                          onClick={e =>
                            this.deleteCourseRelation(e, course, relatedCourse)
                          }>
                      ...
                      <span>
                        <Button
                          ...
                          color="secondary"
                          className={
                            course['displayAddButton']
                              ? classes.approveButton
                              : classes.hidden
                          }
                          onClick={() =>
                            this.displayAddDependencySelect(course)
                          }
                      ...
                      <FormControl
                        variant="outlined"
                        className={
                          course['displayCourseDepenedencySelect']
                            ? classes.formControl
                            : classes.hidden
                        }>
                        <InputLabel
